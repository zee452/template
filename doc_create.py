...
#  WorkDoc = 137
  ...
  sss = ''
  pr_cell = ''
  row = 0  # номер строки параметра замены
...
#=================поиск параметров в строке и запись их в БД============================================
  def ParAdd(s,sp='',doc=False):
     global WorkDoc, row, sel
     n = 0
     j = 0
     m = 0
     ret = -1
        ...
        if m != -1:
            ...
            if doc:
               sss = "'"+sp+"'"
                ...
                sss = "'"+s[j: m-1]+"'"
            if re.search(r"[а-яА-Я]",sss) == None:
                sss = "'" "'"
            sa = 'select docp_v,docp_r,docp_c,doc_id from docp ues (' + ss + ',' + sss.strip() + \
            ...
            for tab in doc.tables:
                ...
                for ro in tab.rows:
                   ...
                   for cell in ro.cells:
                       sel += 1
                       if cell.text not in [pr_cell,'.',',']:
                          ParAdd(cell.text,pr_cell,True)
                    #      print(cell.text)
                          pr_cell = cell.text
#=======================xlrd=======================================================
      if ext[1] =='.xls':
